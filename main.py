...
#   1. Build its tool + create_agent, like flight_agent/hotel_agent below.
...
def build_app():
    """Build the compiled multi-agent graph, reading credentials from the environment.

    Called lazily (not at import time) so the frontend can collect API keys from
    the user first and set them into os.environ before calling this. Reads
    GROQ_API_KEY / DATABASE_URL here, the same way search_flights/tavily_search
    read AVIATIONSTACK_API_KEY / TAVILY_API_KEY at call time.
    """
    llm = ChatGroq(
