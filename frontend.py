...
# Routing-only node — no user-facing output, so it's skipped in the pipeline view.
SKIP_NODES = {"supervisor"}
    ...
    if not user_query.strip():
        ...
        for chunk in app.stream(
            ...
            for node_name, state_update in chunk.items():
                ...
                if "agent_usage" in state_update:
                    collected["agent_usage"] = merge_usage(collected["agent_usage"], state_update["agent_usage"])

                if node_name in SKIP_NODES:
                    continue

                icon, label = AGENT_META.get(node_name, ("🔧", node_name))
                collected["agents_run"] += 1
                ...
                with st.status(f"{icon}  {label}", state="complete", expanded=True):
                    if node_name == "flight_agent":
                        ...
                        text = state_update.get("final_response", "")
                        collected["final_response"] = text
                        st.markdown(text or "_No final response._")

        total_calls = sum(s["calls"] for s in collected["agent_usage"].values())

        # Metrics
        st.markdown(f"""
        <div class="metric-row">
            <div class="metric-box"><div class="metric-val">{collected['agents_run']}</div><div class="metric-lbl">Agents Run</div></div>
            <div class="metric-box"><div class="metric-val">{total_calls}</div><div class="metric-lbl">LLM Calls</div></div>
            <div class="metric-box"><div class="metric-val">✅</div><div class="metric-lbl">Status</div></div>
        </div>
        """, unsafe_allow_html=True)

        # Token usage by agent
        if collected["agent_usage"]:
            st.markdown("<div class='sec-head'><span>📊 Token Usage by Agent</span></div>",
                        unsafe_allow_html=True)

            rows = ""
            totals = {"calls": 0, "input_tokens": 0, "output_tokens": 0, "total_tokens": 0}
            for agent_key, stats in collected["agent_usage"].items():
                icon, label = AGENT_META.get(agent_key, ("🔧", agent_key))
                rows += (
                    "<tr>"
                    f"<td style='padding:0.5rem 1rem;'>{icon} {label}</td>"
                    f"<td style='padding:0.5rem 1rem;text-align:center;'>{stats['calls']}</td>"
                    f"<td style='padding:0.5rem 1rem;text-align:center;'>{stats['input_tokens']}</td>"
                    f"<td style='padding:0.5rem 1rem;text-align:center;'>{stats['output_tokens']}</td>"
                    f"<td style='padding:0.5rem 1rem;text-align:center;'>{stats['total_tokens']}</td>"
                    "</tr>"
                )
                for key in totals:
            ...
            rows += (
                "<tr style='font-weight:700;border-top:1px solid #1e2e44;'>"
                "<td style='padding:0.5rem 1rem;'>Total</td>"
                f"<td style='padding:0.5rem 1rem;text-align:center;'>{totals['calls']}</td>"
                f"<td style='padding:0.5rem 1rem;text-align:center;'>{totals['input_tokens']}</td>"
                f"<td style='padding:0.5rem 1rem;text-align:center;'>{totals['output_tokens']}</td>"
                f"<td style='padding:0.5rem 1rem;text-align:center;'>{totals['total_tokens']}</td>"
                "</tr>"
            )

            st.markdown(f"""
            <div class="final-card" style="padding:0;">
            <table style="width:100%;border-collapse:collapse;font-size:0.88rem;">
            <thead>
            <tr style="background:#0e1a2b;">
                <th style="text-align:left;padding:0.6rem 1rem;color:#7ab8f5;">Agent</th>
                <th style="padding:0.6rem 1rem;color:#7ab8f5;">Calls</th>
                <th style="padding:0.6rem 1rem;color:#7ab8f5;">Input Tokens</th>
                <th style="padding:0.6rem 1rem;color:#7ab8f5;">Output Tokens</th>
                <th style="padding:0.6rem 1rem;color:#7ab8f5;">Total Tokens</th>
            </tr>
            </thead>
            <tbody>
            {rows}
...
## 📊 Token Usage by Agent
{chr(10).join(
    f"- {agent}: {s['calls']} calls, {s['input_tokens']} in / {s['output_tokens']} out / {s['total_tokens']} total tokens"
    for agent, s in collected['agent_usage'].items()
) or 'N/A'}
